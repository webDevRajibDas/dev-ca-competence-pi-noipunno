...
<div class="container">
    <div class="row justify-content-center">
            ...
            <div class="shadow-sm p-2 rounded my-2 custom-card">
                <div class="">
                    <form method="POST" action="{{ route("bi-similarity.store") }}" enctype="multipart/form-data">
                        @csrf

                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    <label for="competence_id" class="form-label">Competence</label>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    ...
                                    <select class="form-select custom-input-form" aria-label="Default select example" name="bi_id" id="bi_id">
                                        ...
                                        @foreach($bis as $bi)
                                        <option value="{{$bi->id}}" class="bi-option class-{{$bi->competence_uid}}">{{app()->getLocale() == 'en' ? $bi->name_en :  $bi->name_bn}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    <label for="similar_competence_id" class="form-label">Competence</label>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    ...
                                    <select class="form-select custom-input-form" aria-label="Default select example" name="similar_bi_id" id="similar_bi_id">
                                        <option value="">Select BI</option>
                                        @foreach($bis as $bi)
                                        <option value="{{$bi->id}}" class="similar-bi-option class-{{$bi->competence_uid}}">{{app()->getLocale() == 'en' ? $bi->name_en :  $bi->name_bn}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('competence_id').addEventListener('change', function() {
            ...
            var biOptions = document.querySelectorAll('.bi-option');

            // Hide all subject options
            biOptions.forEach(function(option) {
                option.style.display = 'none';
            });

            // Show subject options that belong to the selected class
            var biOptions = document.querySelectorAll('.bi-option.class-' + competenceId);
            biOptions.forEach(function(option) {
