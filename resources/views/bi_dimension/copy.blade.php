...
@section('content')
    <div class="custom-card p-3 shadow-sm">
        <form method="GET" action="{{ route('bi-dimension.copy') }}" enctype="multipart/form-data">
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        <select class="form-select custom-input-form" aria-label="Default select example" name="limit">
                            <option value="">BI ডাইমেনশন পরিমান</option>
    ...
    <div class="">
        ...
        @if ($errors->has('biDimensionArr'))
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong>{{ $errors->first('biDimensionArr') }}</strong>
        ...
        <div class="">
            <div class="table-responsive table-container shadow p-2">
                <table id="dimension_table" class="custom-table myTable">
                    ...
                    <form method="post" action="{{ route('bi-dimension.copyStore') }}" id="copyForm">
                        @csrf
                        <tbody>
                            <?php $i = 0; ?>
                            @forelse ($dimensions as $key => $item)
                                ...
                                <tr>
                                    @if (request()->session != date('Y'))
                                        <td>
                                            <input type="checkbox" class="checkItem" name="biDimensionArr[]"
                                                value={{ $item->uid }}>
                                        </td>
                                    @endif
                                    <td>
