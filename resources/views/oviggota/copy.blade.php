...
@section('content')
    <div class="custom-card p-3 shadow-sm">
        <form method="GET" action="{{ route('oviggota.copy') }}" enctype="multipart/form-data">
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        <select class="form-select custom-input-form" aria-label="Default select example" name="limit">
                            <option value="">অভিজ্ঞতা পরিমান</option>
    ...
    <div class="">
        ...
        @if ($errors->has('oviggotaArr'))
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong>{{ $errors->first('oviggotaArr') }}</strong>
        ...
        <div class="">
            <div class="table-responsive table-container shadow p-2">
                <table id="oviggota_table" class="custom-table">
                    ...
                    <form method="post" action="{{ route('oviggota.copyStore') }}" id="copyForm">
                        @csrf
                        <tbody>
                            <?php $i = 0; ?>
                            @forelse ($oviggotas as $key => $item)
                                <?php $i++; ?>
                                <tr>
                                    @if (request()->session != date('Y'))
                                        <td>
                                            <input type="checkbox" class="checkItem" data-oviggota-id={{ $item->uid }}
                                                name="oviggotaArr[]" value={{ $item->uid }}>
                                        </td>
                                    @endif
                                    <td>
                                        {{ $i }}
                            ...
                            @empty
                                <tr>
                                    <td>
                                        ...
                                        @else
                                            <span>ইতোমধ্যে {{ request()->session ?? date('Y')-1 }} এর সমস্ত ডাটা কপি সম্পন্ন হয়ে
                                                গেছে।</span>
                                        @endif
                                    </td>
                                </tr>
                ...
                @if (request()->limit !== 'all')
                    <div class="pagination justify-content-end">
                        {{ $oviggotas->links('vendor.pagination.bootstrap-4') }}
                    </div>
                @endif
            </div>
        </div>
