...
<div class="container">
    <div class="row justify-content-center">
            ...
            <div class="shadow-sm p-2 rounded my-2 custom-card">
                <div class="">
                    <form method="POST" action="{{ @$oviggota ? route('oviggota.update', @$oviggota->uid) : route("oviggota.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    ...
                                    <select class="form-select custom-input-form" aria-label="Default select example" name="class_id" id="class_id">
                                        ...
                                        <option value="{{$class->class_id}}" {{ @$oviggota->class_id == $class->class_id ? 'selected' : '' }}>{{app()->getLocale() == 'en' ? $class->name_en :  $class->name_bn}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    ...
                                    <input type="text" class="custom-input-form" aria-describedby="oviggota_no" value="{{ @$oviggota->oviggota_no }}"
                                        name="oviggota_no">
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 input-container">
                                    ...
                                    <input type="text" class="custom-input-form" aria-describedby="oviggota_title" value="{{ @$oviggota->oviggota_title }}"
                                        name="oviggota_title">
                                </div>
                            </div>
    ...
    $(document).on('change', '#subject_uid', function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                edit_id:"{{  @$oviggota->uid }}"
            },
            success: function(response) {
                var html ='';
                if (response) {
                    $.each(response.chapters, function(index, chapter) {
                        let checked = response.selected_chapter.includes(chapter.uid);
                        html += `
                            <div class="form-group form-check">
                            <input type="checkbox" class="form-check-input" id="chapter_uid[${chapter.uid}]" name="chapter_uid[${chapter.uid}]" ${(checked)?('checked'):('')}>
                            <label class="form-check-label" for="chapter_uid[${chapter.uid}]">${chapter.name}</label>
                            </div>
                        `;
                    });
                }
